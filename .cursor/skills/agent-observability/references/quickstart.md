...
将 `templates/settings-hook.json` 合并到 `~/.cursor/settings.json` 或 `<project>/.cursor/settings.json`。
...
**放哪**：默认 `<project_root>/.cursor/agent-observability/pricing.overrides.json`
（`<project_root>` 是 `.cursor/` 所在的那一层，例如 `/Users/me/my-project`）。

刻意放在 `skills/` 树**之外**，原因有三：

- `scripts/build-classic-hosts.py` 会整棵同步 `.cursor/skills` 到 `.claude/` `.cursor/`，
