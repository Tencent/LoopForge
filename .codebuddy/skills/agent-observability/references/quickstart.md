...
**放哪**：默认 `<project_root>/.codebuddy/agent-observability/pricing.overrides.json`
（`<project_root>` 是 `.codebuddy/` 所在的那一层，例如 `/Users/me/my-project`）。

刻意放在 `skills/` 树**之外**，原因有三：

- `scripts/build-classic-hosts.py` 会整棵同步 `.codebuddy/skills` 到 `.claude/` `.cursor/`，
